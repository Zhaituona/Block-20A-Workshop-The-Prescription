...
describe("The document", () => {
  ...
  test("displays the correct cost for the user's input values after clicking the calculate button", () => {
    ...
    // Manually calculate the expected cost for verification
    const priceValue = parseFloat(price.value);
    const refillsValue = parseInt(refills.value, 10);
    let expectedCost = priceValue * refillsValue;

    if (subscribed.checked) {
      expectedCost *= 0.75; // Assuming 25% discount for subscription
    }

    if (coupon.checked) {
      expectedCost -= 10; // Assuming $10 off for coupon
    }

    // Ensure the cost is not negative
    expectedCost = Math.max(expectedCost, 0);

    // Check the expected cost value
    expect(costOutput.textContent).toBe(`$${expectedCost.toFixed(2)}`);
  });
});
